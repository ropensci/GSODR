...
```{r SRTM 90m vs Reported Elevation, echo=TRUE, fig.cap="GSOD Reported Elevation versus CGIAR-CSI SRTM Elevation"}
plot(stations$ELEV.M.SRTM.90m ~ stations$ELEV.M)
```

```{r Buffered SRTM 90m vs Reported Elevation, echo=TRUE, fig.cap="GSOD Reported Elevation versus CGIAR-CSI SRTM Buffered Elevation"}
...
elevation checks are extremely minor. We can assume that the values of the 90m
check are sufficient and provide them as a "corrected" elevation option with
the cleaned GSOD station data provided by this package.

## Cleanup and save data to disk

Finally, drop the buffered station field, write the .rda file to disk (I've
already checked for the optimal compression of the rda file using
`tools::checkRdaFiles()`, using bzip2) and clean up the downloaded Natural Earth
Data files.

```{r cleanup and .rda file creation, messages=FALSE}
