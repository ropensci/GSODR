...
The final data frame for distribution with _GSODR_ includes the new elevation 
values in the `ELEV_M_SRTM_90m` field along with the cleaned
"isd-history.csv" data that removes stations that do not have valid x, y
locations as a `data.table` object.

```{r cleanup_and_.rda_file_creation, message=FALSE, echo=TRUE, warning=FALSE, error=FALSE}

setDT(isd_history)
setkey(isd_history, "STATION")
