...
-   `get_GSOD()` - queries and transfers files from the FTP server, reformats them and returns a `data.frame()` object in R or saves a file to disk with options for a GeoPackage or comma seperated values (CSV) file,  
...
-   `get_station_list()` - downloads the latest station list from the NCDC FTP server.
