...
downloading and cleaning of data from the "[Global Surface Summary of the Day
...
Data are formatted for easy use in R, returned as a `data.frame` object in R
summarising each year by station, with options to save as a Comma Separated
Value (CSV) file or as a spatial GeoPackage (GPKG) file, implemented by most
major GIS softwares.
...
millimetres and Fahrenheit to Celsius.
...
file, PHL-2010.gpkg, in the user's home directory with a maximum of five missing
days per station allowed and no CSV file.
