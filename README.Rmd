...
and working properly this may not always happen.
...
```{r example_3, eval=TRUE, echo=FALSE}
...
```{r example_3.2, eval=TRUE, fig.cap="Comparison of GSOD daily values and average monthly values with CHELSA climate monthly values"}
library(plyr)
library(dplyr)
library(ggplot2)
library(reshape2)
...
clim_temp = GSOD_clim[GSOD_clim$STNID %in% pnts$STNID, paste(c("STNID", cnames))]
...
temp <- join(pnts@data, clim_temp_df, type = "left")
temp <- temp %>% 
  group_by(MONTH) %>% 
  mutate(AVG_DAILY_TEMP = round(mean(TEMP), 1))

df_melt <- na.omit(melt(temp[, c("STNID", "DATE", "CHELSA_TEMP", "TEMP",
                                 "AVG_DAILY_TEMP")],
                id = c("DATE", "STNID")))
...
The function, `get_GSOD`, returns a `data.frame` object in R and 
can also save a Comma Separated Value (CSV) file or
GeoPackage (GPKG) file for use in a GIS. Station data are merged with weather
