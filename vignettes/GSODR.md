...
The GSOD data are comprised of a global set of data from weather stations. To visualise where these stations are located we can fetch the station metadata and plot it in a map. Using [ggplot2](https://CRAN.R-project.org/package=ggplot2) it is possible to plot the station locations using alpha transparency to see the densest part of the network. The resulting map shows only stations with valid geo-locations after filtering.
