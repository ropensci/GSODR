...
## Find stations near a particular latitude and longitudinal coordinate

The `stations` data that is included with the GSODR package can be converted into
a spatial object and spatial queries can be performed. For instance.
...
coordinates(sp_stations) <- ~LON + LAT
class(sp_stations)


sp_stations <- stations
coordinates(sp_stations) <- c("LON", "LAT")
proj4string(sp_stations) <- CRS("+init=epsg:4326")
class(sp_stations)
...
nearby <- apply(spDists(sp_stations, pt), 2, 
