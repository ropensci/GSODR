...
the NCDC website and removes stations that are missing data, like this.

Using this data we can plot the station locations that are included in GSOD that
provide valid geo-locations after cleaning as shown below.
...
However, the original list is updated somewhat regularly by the NCDC. If you
would like to use the very latest station list, you may do so by using the
`refresh` parameter in `get_GSOD()`. This list will only be available for the R
session in which you have specified the option to refresh the station list and
downloaded data, it will not overwrite the default list available with the
package.

```{r refresh_stations, eval=FALSE}
