...
```
## Error in ogrListLayers(dsn = path.expand("~/PHL-2010.gpkg")): Cannot open data source
```
...
```
## Error in readOGR(dsn = path.expand("~/PHL-2010.gpkg"), layers[1]): object 'layers' not found
```
...
```
## Error in paste(pnts$YEAR, pnts$MONTH, pnts$DAY, sep = "-"): object 'pnts' not found
```
...
```
## Error in nrow(pnts): object 'pnts' not found
```
...
```
## Error in as.POSIXct(endTime): object 'pnts' not found
```
...
```
## Error in kml(tmp_ST, dtime = 24 * 3600, colour = TEMP, shape = shape, : object 'tmp_ST' not found
```
...
```
## Skipping install of 'GSODRdata' from a github remote, the SHA1 (fd505088) has not changed since last install.
...
```
## Error in CHELSA$STNID %in% pnts$STNID: object 'pnts' not found
```
...
```
## Error in data.frame(STNID = rep(clim_temp$STNID, 12), MONTHC = as.vector(sapply(1:12, : object 'clim_temp' not found
```
...
```
## Error in paste(pnts$MONTH): object 'pnts' not found
```
...
```
## Error in left_join(pnts@data, clim_temp_df, by = c("STNID", "MONTHC")): object 'pnts' not found
```
...
```
## Error in eval(lhs, parent, parent): object 'temp' not found
```
...
```
## Error in melt(temp[, c("STNID", "DATE", "CHELSA_TEMP", "TEMP", "AVG_DAILY_TEMP")], : object 'temp' not found
```
...
```
## Error in ggplot(df_melt, aes(x = DATE, y = value)): object 'df_melt' not found
```
