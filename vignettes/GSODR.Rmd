...
data provided by the US National Centers for Environmental Information (NCEI) are a valuable source of weather data with global coverage. However, the data files are cumbersome and difficult to work with. The _GSODR_ package aims to make it easy to find, transfer and format the data you need for use in analysis. The _GSODR_ package provides four main functions for facilitating this:
...
Additional climate data, [_GSODdata_](https://github.com/adamhsparks/GSODRdata), formatted for use with GSOD data provided by _GSODR_ are available as an R package installable through GitHub due to the package size, 5.1Mb, being too large for CRAN.
