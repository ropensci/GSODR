...
As a first step, we'll use the [_raster_](https://cran.r-project.org/package=raster) package to retrieve data from [GADM](http://www.gadm.org/) that will provide the provincial spatial data for the survey area. We will then use this to find the centroid of the area of interest, which will be used to find the nearest stations. Using `raster::getData()` fetch level 0 (national) and 1 (provincial) data for the Philippines. While fetching them, convert them to `sf` objects using `sf::st_as_sf()` from the [_sf_](https://r-spatial.github.io/sf/index.html) package.

```{r, eval=TRUE}
library(raster)
library(sf)

RP0 <- st_as_sf(getData(country = "Philippines", level = 0))
RP1 <- st_as_sf(getData(country = "Philippines", level = 1))
```

Now we will select the provinces involved in the survey and make a new object called `Central_Luzon` from the provincial level data, `RP1`. To do this, we will use the [_dplyr_]() package to filter the values we want.

```{r, eval=TRUE, message=FALSE}

library(dplyr)

Central_Luzon <- st_as_sf(RP1)

Central_Luzon <- filter(RP1, NAME_1 %in% c("Pampanga", 
                                           "Tarlac",
                                           "Pangasinan",
                                           "La Union",
                                           "Nueva Ecija",
...
First we'll use `ms_simplify()` from [_rmapshaper_](https://github.com/ateucher/rmapshaper) to simplify the map of the Philippines to make the map generation in the next few steps quicker, and then convert it to an `sf` object as well.

```{r, eval=TRUE}
library(rmapshaper)

RP0 <- ms_simplify(RP0, keep = 0.05)
...
CL_names <- st_transform(Central_Luzon, 25393) %>% 
  st_centroid() %>% 
...
CL_names$label <- Central_Luzon$NAME_1

# Main Map
p1 <- ggplot() + 
  geom_sf(data = Central_Luzon,
          colour = "grey10",
          fill = "#fff7bc") +
  geom_label(data = CL_names, aes(x = X, y = Y, label = label), 
             size = 3,
             colour = "grey20") +
...
v2 <- viewport(width = 0.28, height = 0.28, x = 0.68, y = 0.78) # plot area for the inset map
print(p1, vp = v1) 
print(p2, vp = v2)
```
